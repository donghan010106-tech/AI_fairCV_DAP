...
def render_dataset():
    ...
    <div class="hero-banner">
        <div class="hero-title">Dataset & EDA</div>
    ...
    with col1:
        st.markdown("""<div class="kpi-card"><div class="kpi-value">24,000</div>
        <div class="kpi-label">Total Profiles</div>
        <div style="font-size:0.68rem;color:#7d8590;margin-top:0.3rem;">19,200 train / 4,800 test</div></div>""", unsafe_allow_html=True)
    with col2:
        st.markdown("""<div class="kpi-card"><div class="kpi-value">60</div>
        <div class="kpi-label">Total Columns</div>
        <div style="font-size:0.68rem;color:#7d8590;margin-top:0.3rem;">demographics, competency, embeddings, text, labels</div></div>""", unsafe_allow_html=True)
    with col3:
        st.markdown("""<div class="kpi-card"><div class="kpi-value">10</div>
        <div class="kpi-label">Occupations</div>
        <div style="font-size:0.68rem;color:#7d8590;margin-top:0.3rem;">across 4 sectors</div></div>""", unsafe_allow_html=True)
    ...
    groups = [
        ...
        ("Face Embeddings", ["face_emb_0 to 19 (20-dim, original)", "blind_face_emb_0 to 19 (20-dim, SensitiveNets)"],
        ...
        ("Labels (3)", ["blind_label — fair score, no demographic penalty", "biased_label_gender — female penalized ~25%", "biased_label_ethnicity — G1 & G3 penalized"],
         "#22c55e", "Continuous to binary via median(blind_label) ~ 0.413"),
    ]
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div class="section-header">Label System & Binarization</div>', unsafe_allow_html=True)

    c1, c2, c3 = st.columns(3)
    label_data = [
        ("blind_label", "Fair Target", "#22c55e",
         "Score based purely on competency and suitability — no demographic penalty. Primary training target for all fusion experiments."),
        ("biased_label_gender", "Gender-Biased", "#f43f5e",
         "Female candidates penalized by ~25% relative to equivalent male candidates."),
        ("biased_label_ethnicity", "Ethnicity-Biased", "#f59e0b",
         "Groups G1 and G3 are penalized relative to G2. Produces the largest fairness gaps — EOO up to 0.321 on LR."),
    ]
    ...
    st.markdown("""
    <div class="amber-box">
        <strong>Binarization Strategy:</strong> All three continuous labels are converted to binary
        using the median of <code style="background:#0d1117;padding:1px 4px;border-radius:3px;">blind_label</code> as threshold (~0.413).
        Same threshold across all three labels ensures comparability.
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="insight-box">
        FairCVdb is intentionally constructed with balanced demographics — ~50/50 gender and ~33/33/33 ethnicity.
        Any score gaps between groups cannot be attributed to class imbalance, making them clear evidence of bias.
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_l:
        ...
        <thead><tr><th>Group</th><th>Type</th><th>Count</th><th>%</th></tr></thead>
        <tbody>
    ...
    with col_r:
        st.markdown("""
        <table class="result-table">
        <thead><tr><th>Sector</th><th>Occupations</th><th>Suitability</th></tr></thead>
        <tbody>
    ...
    feats = [
        ("suitability",      "Job-category match score (0.25/0.5/0.75/1.0)", "Directly linked to blind_label"),
    ...
    <tbody>
    """ + "".join(f"<tr><td class='mono'>{n}</td><td>{d}</td><td style='color:#7d8590;font-size:0.8rem;'>{note}</td></tr>"
                  for n, d, note in feats) + """
    </tbody></table>
    """, unsafe_allow_html=True)
